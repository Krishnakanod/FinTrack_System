...
test.beforeEach(async ({ page }) => {
  ...
  await page.addInitScript(() => {
    ...
    // Also pre-set the Zustand store directly by intercepting its initialization
    const originalSetItem = window.localStorage.setItem;
    // This ensures the store hydrates correctly
  });
});
...
test.describe('Expenses Page', () => {
  ...
  test('add dialog opens with tabs', async ({ page }) => {
    await page.goto(`${BASE_URL}/dashboard/expenses`);
    await page.waitForLoadState('networkidle');
